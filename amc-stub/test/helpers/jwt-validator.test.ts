...
describe("jwt validator tests", () => {
  ...
  [AMCScopes.ACCOUNT_DELETE, AMCScopes.PASSKEY_CREATE].forEach((scope) => {
    it(`should validate the composite JWT for scope ${scope}`, async () => {
      ...
      expect(payload.govuk_signin_journey_id).to.equal(
        TEST_CONSTANTS.JOURNEY_ID
      );
  ...
  [
    ...
    it(`should return an error string when the client assertion payload ${scope} is invalid`, async () => {
      ...
      expect(await validateCompositeJWT(JWT)).to.equal(
        "The client assertion JWT payload scope should be one of account-delete, passkey-create"
      );
    });
  });
