...
async function getPublicSigningKey(nestedJws: string): Promise<CryptoKey | KeyObject | JWK | Uint8Array> {
...
async function getPublicKeyFromJwks(kid: string): Promise<CryptoKey | KeyObject | JWK | Uint8Array> {
  ...
  for (const k of jwks.keys) {
    if (k.kid === kid) {
      ...
      // In jose v6.x, importJWK requires an algorithm when jwk.alg is not present
